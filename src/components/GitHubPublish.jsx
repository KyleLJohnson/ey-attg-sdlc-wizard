...
export default function GitHubPublish({ files, projectName, mode = 'greenfield', existingRepo = '' }) {
  ...
  // Strategy:
  //   1. Create repo with auto_init:true so GitHub initialises the git object
  //      store (without this every /git/trees call returns 409 "Git Repository
  //      is empty", no matter how many retries).
  //   2. Read the SHA of the auto_init commit so we can parent our commit on it.
  //   3. POST /git/trees — retry with backoff because GitHub's object store can
  //      still return a transient 404 for ~1-2 s after the ref becomes readable.
  //   4. Create our commit (parent = auto_init SHA) and force-update the branch.
  async function createAndPush() {
    ...
    try {
      // ── Step 1: Create repo (auto_init seeds the git object store) ───────
      ...
      // ── Step 2: Read auto_init commit SHA ────────────────────────────────
      ...
      for (let attempt = 1; attempt <= 6; attempt++) {
        ...
        const refRes = await ghFetch(
          `/repos/${repo.full_name}/git/ref/heads/${defaultBranch}`, token);
        if (refRes.ok) {
          const ref = await refRes.json();
          initialCommitSha = ref.object.sha;
          break;
        }
        if (attempt === 6) throw new Error('Could not read initial repo ref after retries.');
      }

      // ── Step 3: Build kit tree — retry on transient 404 ──────────────────
      // Even after the ref is readable the git object store can briefly return
      // 404 for new objects; retry with backoff to ride out that window.
      setProgress(`Building git tree (${fileCount} files)…`);
      const treeItems = Object.entries(files).map(([path, content]) => ({
        path, mode: '100644', type: 'blob', content,
      }));
      ...
      for (let attempt = 1; attempt <= 6; attempt++) {
        if (attempt > 1) {
          setProgress(`Building git tree… (attempt ${attempt}/6)`);
        ...
        if (treeRes.ok) { tree = await treeRes.json(); break; }
        if (attempt === 6) {
      ...
      // ── Step 4: Create kit commit (parent = auto_init README commit) ─────
      setProgress('Creating commit…');
      ...
      const updateRefRes = await ghFetch(
        `/repos/${repo.full_name}/git/refs/heads/${defaultBranch}`, token, {
          method: 'PATCH',
