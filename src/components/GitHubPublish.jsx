...
export default function GitHubPublish({ files, projectName, mode = 'greenfield', existingRepo = '' }) {
  ...
  // ── Create repo + push all files ─────────────────────────────────────────
  // Strategy: create empty repo, then POST all files as a single git tree.
  // GitHub occasionally returns 404 on /git/trees straight after repo creation
  // while the git backend propagates — we retry up to 5 times with backoff.
  async function createAndPush() {
    ...
    try {
      ...
      // ── Step 2: Build kit tree — retry until git backend is ready ─────────
      // New repos take up to ~2 s for the git object store to become available.
      setProgress(`Building git tree (${fileCount} files)…`);
      const treeItems = Object.entries(files).map(([path, content]) => ({
        path, mode: '100644', type: 'blob', content,
      }));
      ...
      for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
        if (attempt > 1) {
          setProgress(`Building git tree… (attempt ${attempt}/${MAX_RETRIES})`);
        ...
        // 404/409 on a new repo — git backend not ready yet, retry
      }
